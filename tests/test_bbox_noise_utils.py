...
from gazelle import utils


def test_zero_jitter_returns_original_normalized_bbox():
    bbox = [0.2, 0.3, 0.4, 0.6]

    assert utils.perturb_normalized_bbox(bbox, 0, np.random.default_rng(123)) == bbox
...
def test_bbox_perturbation_is_seeded_and_valid():
    ...
    perturbed_a = utils.perturb_normalized_bbox(bbox, 20, rng_a)
    perturbed_b = utils.perturb_normalized_bbox(bbox, 20, rng_b)
